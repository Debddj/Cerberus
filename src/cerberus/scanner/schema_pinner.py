...
import json
import aiosqlite
from cerberus.proxy.models import SchemaPin
...
class SchemaPinner:
    ...
    async def verify_or_pin(self, server_url: str, tool_name: str, desc: str, schema: dict) -> tuple[bool, str | None]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            stored_d_hash, stored_s_hash, version = row
            if stored_d_hash != d_hash or stored_s_hash != s_hash:
                return False, f"Hash mismatch detected! Potential tool poisoning or rug-pull on '{tool_name}'"
