...
class LethalTrifectaDetector:
    ...
    PRIVATE_DATA_KEYWORDS = {"read", "query", "select", "fetch_db", "get_secret", "file_read", "credentials"}
    UNTRUSTED_CONTENT_KEYWORDS = {"inbox", "scrape", "fetch_url", "search_web", "parse_issue", "receive_email"}
    EXTERNAL_EGRESS_KEYWORDS = {"http_post", "send_email", "webhook", "upload", "publish", "post_message"}
    ...
    def check_session_tools(cls, tool_names: Iterable[str]) -> tuple[bool, dict[str, list[str]]]:
        ...
        is_trifecta = (
            len(breakdown["private_data"]) > 0 and
            len(breakdown["untrusted_content"]) > 0 and
            len(breakdown["external_egress"]) > 0
        )
