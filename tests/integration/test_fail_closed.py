...
def test_fail_closed_behavior():
    async def _run():
        ...
        low_event = ToolCallEvent(
            session_id="s1", agent_id="a1", tool_name="read", tool_server="srv", risk_score=0.2
        )
        dec_low, _ = await client.evaluate(low_event)
        assert dec_low == EventDecision.ALLOW

        high_event = ToolCallEvent(
            session_id="s1", agent_id="a1", tool_name="post", tool_server="srv", risk_score=0.8
        )
