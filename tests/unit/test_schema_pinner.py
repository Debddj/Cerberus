...
def test_schema_pin_and_verify(tmp_path):
    async def _run():
        ...
        # Initial pin
        ok, err = await pinner.verify_or_pin(
            "http://srv", "tool_a", "Math tool", {"type": "object"}
        )
        assert ok is True
        assert err is None

        # Same definition -> Valid
        ok2, err2 = await pinner.verify_or_pin(
            "http://srv", "tool_a", "Math tool", {"type": "object"}
        )
        assert ok2 is True
        assert err2 is None
        ...
        ok3, err3 = await pinner.verify_or_pin(
            "http://srv", "tool_a", "Math tool with injected prompt", {"type": "object"}
        )
        assert ok3 is False
